...
function writeMarkerConfig(projectPath) {
  try {
    ...
    const projectDir = toPosix(path.resolve(projectPath));
...
const { parsePage, serializePageMarked, resolveChunks, markChunkHtml, previewScopeFromFile } = require(${JSON.stringify(
      parserPath
    )});
...
const avbMarkers = {
  ...
  load(id) {
    ...
    if (query) {
      ...
      const markerPath = params.get('avb');
      const scope = params.get('avbs');
      if (!markerPath || !/^[\\d.]+$/.test(markerPath) || !scope) return null;
      try {
    ...
    if (!normalizedFile.endsWith('.astro') || !normalizedFile.startsWith(SRC_DIR + '/')) return null;
    try {
      const scope = previewScopeFromFile(PROJECT_DIR, normalizedFile);
